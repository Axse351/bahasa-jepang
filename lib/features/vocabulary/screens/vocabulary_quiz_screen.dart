...
class VocabularyQuizScreen extends StatefulWidget {
  final int level;
  const VocabularyQuizScreen({super.key, this.level = 5});
...
class _VocabularyQuizScreenState extends State<VocabularyQuizScreen> {
  ...
  List<VocabularyItem> _allVocab = [];
  List<VocabularyItem> _questions = [];
  int _currentIndex = 0;
  int _correctCount = 0;
  int _wrongCount = 0;
  ...
  Future<void> _loadData() async {
    try {
      final vocab = await _repo.loadVocabulary(level: widget.level);
      vocab.shuffle(_random);
      setState(() {
        _allVocab = vocab;
        _questions = vocab.take(_sessionSize).toList();
        _loading = false;
      });
      _prepareOptions();
    } catch (e) {
  ...
  void _prepareOptions() {
    ...
    final distractorPool = _allVocab.where((v) => v.id != correct.id).toList()
      ..shuffle(_random);
    final distractors = distractorPool.take(3).map((v) => v.arti).toList();
  ...
  void _selectAnswer(String answer) {
    ...
    _repo.logAttempt(vocabularyId: correct.id, correct: isCorrect);
        ...
        child: Builder(
          builder: (context) {
            ...
            if (_questions.isEmpty) {
              return const Center(
                child: Text('Belum ada data kosakata untuk kuis.'),
              );
            }
              ...
              child: ElevatedButton(
                onPressed: () {
                  setState(() {
                    ...
                    _correctCount = 0;
                    _wrongCount = 0;
                    _allVocab.shuffle(_random);
                    _questions = _allVocab.take(_sessionSize).toList();
                  });
                  _prepareOptions();
                },
